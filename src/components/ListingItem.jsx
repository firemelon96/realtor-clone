    ...
    <li className='relative bg-white flex flex-col justify-between items-center shadow-md hover:shadow-xl rounded-md overflow-hidden transition-shadow duration-150 m-[10px]'>
        <Link className='contents' to={`/category/${listing.type}/${id}`}>
            ...
            <div className="w-full p-[10px] ">
                ...
                <p className='text-[#457b9d] mb-2 font-semibold '>${listing.offer ? listing.discountedPrice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") : listing.regularPrice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} {listing.type === "rent" && " / Month"}</p>
        ...
        {onDelete && (
            <FaTrash className='absolute right-2 bottom-2 h-[14px] cursor-pointer text-red-600' onClick={()=>onDelete(listing.id)}/>
        )}
        {onEdit && (
            <FaEdit className='absolute right-7 bottom-2 h-[14px] cursor-pointer' onClick={()=>onEdit(listing.id)} />
        )}
    </li>
  )
}
